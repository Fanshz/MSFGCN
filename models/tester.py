...
def model_inference(sess, pred, inputs, batch_size, n_his, n_pred, step_idx, min_va_val, min_val):
    '''
    Model inference function.模型推理函数
    :param sess: tf.Session().
...
def model_test(inputs, batch_size, n_his, n_pred, save_result, inf_mode, load_path='./output/models/'):
    '''
    ...
    with tf.Session(graph=test_graph) as test_sess:
        ...
        file_s = open(save_result, 'a')
        for ix in tmp_idx:
            te = evl[ix - 2:ix + 1]
            print(f'Time Step {ix + 1}: MAPE {te[0]:7.3%}; MAE  {te[1]:4.3f}; RMSE {te[2]:6.3f}.')
        print(f'Model Test Time {time.time() - start_time:.3f}s')
        file_s.writelines('New Result!' + '\n')
        file_s.writelines(f'Time Step {ix + 1}: MAPE {te[0]:7.3%}; MAE  {te[1]:4.3f}; RMSE {te[2]:6.3f}. Model Test Time {time.time() - start_time:.3f}s' + '\n')
