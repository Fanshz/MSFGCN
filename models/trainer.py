...
def model_train(inputs, blocks, args, sum_path='./output/tensorboard/'):
    ...
    :param blocks: list, channel configs of st_conv blocks.  <class 'list'>: [[1, 32, 64], [64, 32, 128]]
    ...
    lr = tf.train.exponential_decay(args.lr, global_steps, decay_steps=5 * epoch_step, decay_rate=0.7, staircase=True)##针对学习率的指数衰减算法
    ...
    with tf.Session() as sess:
        ...
        for i in range(epoch):##epoch训练轮数
            start_time = time.time()
            for j, x_batch in enumerate(######用batch_size分开
                    gen_batch(inputs.get_data('train'), batch_size, dynamic_batch=True, shuffle=True)):
                summary, _ = sess.run([merged, train_op], feed_dict={x: x_batch[:, 0:n_his + 1, :, :], keep_prob: 1.0})##训练时再用n_his分开
                # print(sess.run())
                # with tf.Session() as sess:
                    # varvar = sess.graph.get_tensor_by_name(f'stn_block_{1}_in/attention_adj:0')#(scope + '/var:0')
                # print(_)
                # for v in tf.trainable_variables():
                #     print(v.name)
                # print(sess.run(attention_adj))
            ...
            va_re_rmse.append(f'{va[2]:6.3f}')
            te_re_rmse.append(f'{te[2]:6.3f}')
            print(f'Epoch {i:2d} Inference Time {time.time() - start_time:.3f}s')

            if (i + 1) % args.save == 0:
                model_save(sess, global_steps, 'STGCN')
        writer.close()
    va_re_rmse = list(map(eval, va_re_rmse))
    te_re_rmse = list(map(eval, te_re_rmse))
    plt.plot(va_re_rmse, color='blue', label='va')
    plt.plot(te_re_rmse, color='red', label='te')
    plt.show()
